...
class DepthAwareTransformer(nn.Module):
    ...
    def forward(
            ...
            depth_pos_embed=None, depth_pos_embed_ip=None, attn_mask=None,
        ...
        if outside_query_embed is not None:
            if self.two_stage or self.use_dab or self.two_stage_dino:
                raise NotImplementedError(
                    "Dedicated outside queries currently require the standard "
                    "one-stage non-DAB MonoDETR query path."
                )
            outside_query_pos, outside_tgt = torch.split(
                outside_query_embed, c, dim=1
            )
            outside_query_pos = outside_query_pos.unsqueeze(0).expand(
                bs, -1, -1
            )
            outside_tgt = outside_tgt.unsqueeze(0).expand(bs, -1, -1)
            outside_reference_points = self.reference_points(
                outside_query_pos
            ).sigmoid()
            outside_init_reference = outside_reference_points
            (
                outside_hs,
                ...
                attn_mask=None,
                query_group_num=1,
                queries_per_group=outside_query_embed.shape[0],
            )
            outside_decoder_outputs = (
                outside_hs,
        ...
        if outside_decoder_outputs is not None:
            return (
                hs,
                init_reference_out,
...
class DepthAwareDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        effective_group_num = (
            self.group_num if query_group_num is None else query_group_num
        )
        if self.training and effective_group_num > 1:
            grouped_query_count = effective_group_num * queries_per_group
            num_noise = num_queries - grouped_query_count
            q_noise = q[:num_noise].repeat(1, effective_group_num, 1)
            k_noise = k[:num_noise].repeat(1, effective_group_num, 1)
            v_noise = v[:num_noise].repeat(1, effective_group_num, 1)
            q = q[num_noise:]
            k = k[num_noise:]
            v = v[num_noise:]
            q = torch.cat(q.split(queries_per_group, dim=0), dim=1)
            k = torch.cat(k.split(queries_per_group, dim=0), dim=1)
            v = torch.cat(v.split(queries_per_group, dim=0), dim=1)
...
class DepthAwareDecoder(nn.Module):
    ...
    def forward(
            self, tgt, reference_points, src, src_spatial_shapes,
            src_level_start_index, src_valid_ratios, query_pos=None,
            src_padding_mask=None, depth_pos_embed=None, mask_depth=None,
            bs=None, depth_pos_embed_ip=None, pos_embeds=None, attn_mask=None,
            query_group_num=None, queries_per_group=50):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output,
                           ...
                           query_group_num=query_group_num,
