...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_3dcenter: float = 1, cost_bbox: float = 1,
                 cost_giou: float = 1, use_outside_center_modeling: bool = False):
        """Creates the matcher
        Params:
    ...
    @torch.no_grad()
    def forward(
            self, outputs, targets, group_num=11, target_box_key=None,
            box_valid_mask_key=None):
        ...
        tgt_3dcenter = torch.cat(
            [v[target_box_key][:, 0: 2] for v in targets]
        )
        ...
        tgt_2dbbox = torch.cat(
            [v[target_box_key][:, 2: 6] for v in targets]
        )
        ...
        tgt_bbox = torch.cat([v[target_box_key] for v in targets])
        if box_valid_mask_key is None and (
                self.use_outside_center_modeling
                and target_box_key == "boxes_3d_representative"):
            box_valid_mask_key = "representative_box_valid_mask"
        if box_valid_mask_key is not None:
            target_box_valid = torch.cat(
                [v[box_valid_mask_key] for v in targets]
            ).bool()
            valid_float = target_box_valid.to(cost_bbox.dtype).unsqueeze(0)
            cost_bbox = cost_bbox * valid_float
            cost_giou = torch.zeros_like(cost_bbox)
            if target_box_valid.any():
                cost_giou[:, target_box_valid] = -generalized_box_iou(
                    box_cxcylrtb_to_xyxy(out_bbox),
                    box_cxcylrtb_to_xyxy(tgt_bbox[target_box_valid]))
        else:
